        ...
        if not self.current_split.exists():  # handles missing splits
            ...
            for i in sub:
                if self.dataset_name == 'ABCD':
                    ...
                    if self.intermediate_vec == 322:
                        # filename = os.path.join(kwargs.get('enigma_path'), i+'/'+i+'.tsv')   
                        # file = pd.read_csv(filename, sep='\t', header=None).values
                        # file = file[20:20 + self.seq_len].T
                        filename = os.path.join(kwargs.get('enigma_path'), i+'/'+i+'.npy')   
                        file = np.load(filename)[20:20+self.seq_len].T                
                    
                
                # quality check of the time-series data
                if file.shape[1] >= self.seq_len:  # ensure the number of time points is greater than or equal to the self.seq_len
                    valid_sub.append(i)  # if this condition is met, the subject ID is added to the valid_sub list
